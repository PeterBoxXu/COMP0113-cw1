            ...
            foreach (LaserElement element in elementsList)
            {
                ...
                if (m_active)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // 设置 LineRenderer 的终点为碰撞点 (设置终点)
                            element.lineRenderer.SetPosition(1, hitInfo2D.point);

                            // 更新火花特效的位置与旋转 (更新火花位置和方向)
                            if( m_assignSparks )
                            {
                                element.sparks.transform.position = hitInfo2D.point; // 设置火花位置
                                element.sparks.transform.rotation = Quaternion.LookRotation( hitInfo2D.normal ); // 设置火花朝向
                            }

                            /*
                            EXAMPLE : 在此处添加你希望执行的功能，
                            例如，当 hitInfoXD.collider 不为 null 时，对目标对象执行某个操作。
                            DoAction();
                            */
                        }
                        else
                        {
                            // 未检测到碰撞时，将终点设置为射线最大距离处 (无碰撞时延伸至最大距离)
                            element.lineRenderer.SetPosition(1, element.transform.position + element.transform.forward * m_currentProperties.m_maxRadialDistance);
                        }
